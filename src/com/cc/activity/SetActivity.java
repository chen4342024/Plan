...
public class SetActivity extends Activity implements OnClickListener{
	...
	protected void onPause() {
		...
		PlanService.instance().WriteVal("remind_way", remind_way_text.getText()
				.toString());
		PlanService.instance().WriteVal("plan_count", plan_count_text.getText()
				.toString());
		PlanService.instance().WriteVal("remind_time", remind_time_text.getText()
				.toString());
	...
	private void initData() {
		...
		plan_count_text.setText(PlanService.instance()
				.ReadVal("plan_count", "5条计划"));
		remind_time_text.setText(PlanService.instance().ReadVal("remind_time",
				"晚上8点"));
	...
	public void onClick(View v) {
		...
		case R.id.plan_is_remind_layout:
			if (bl_plan) {
				plan_is_remind_image
						.setBackgroundResource(R.drawable.set_unselect);
				bl_plan = false;
			} else {
				plan_is_remind_image
						.setBackgroundResource(R.drawable.set_select);
				bl_plan = true;
			}
			break;
		case R.id.memo_is_remind_layout:
			if (bl_memo) {
				memo_is_remind_image
						.setBackgroundResource(R.drawable.set_unselect);
				bl_memo = false;
			} else {
				memo_is_remind_image
		...
		case R.id.remind_music_layout:
			Toast.makeText(SetActivity.this, "点击了提醒音乐设置", Toast.LENGTH_LONG)
					.show();
			break;
		case R.id.password_set_layout:
			Toast.makeText(SetActivity.this, "点击了密码设置", Toast.LENGTH_LONG)
					.show();
			break;
		default:
			break;
